...
namespace _root.Script.Ingame
{
    public class FieldSetter : MonoBehaviour
    {
        ...
        public List<IngameCard> GetStudentCards()
        {
            return fieldCards.Where(x => x.type == IngameCardType.Student).ToList();
        }
        ...
        public void UpdateField(List<GameCard> cards)
        {
            ...
            var addedIngameCards = new List<IngameCard>();
        ...
        {
            foreach (var ingameCard in removed) ingameCard.Show(false, true);

            yield return new WaitForSeconds(0.7f);

            UpdateFieldCardPos();
            foreach (var ingameCard in field) ingameCard.Show(true);
        }
